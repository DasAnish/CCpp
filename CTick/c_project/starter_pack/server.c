...
int main(int argc, char *argv[]) {
  ...
  for(;;) { // Error 3: This loop is not closed

    if ( (connfd = accept(listenfd, (struct sockaddr *) NULL, NULL)) < 0 ) { // Error 1: accepts from null?
    ...
    while(!feof(fp)) {
      ...
      while(w<r) { // Error 2: w is uninitialised
